...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            y_g = inputs.y_g;
            u_gtg = [x_g-x; y_g-y]
            u_gtg_n = sqrt ( (u_gtg(1,:))^2 + (u_gtg(2,:))^2 ) ;
            u_gtg_f = u_gtg / u_gtg_n    %% Normalised Version of The Vector to make it no between zero --> 1
            
            %% START CODE BLOCK %%   %% Done %%
            alpha = 0.4 ;
            ux = (alpha * u_gtg_f(1,:))+ ((1-alpha)* u_ao(1,:)) ;
            uy = (alpha * u_gtg_f(2,:))+ ((1-alpha)* u_ao(2,:)) ;
