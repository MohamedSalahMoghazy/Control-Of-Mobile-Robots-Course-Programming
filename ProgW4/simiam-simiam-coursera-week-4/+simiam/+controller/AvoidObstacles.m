...
% see the LICENSE file included with this software
    ...
    methods
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            u_i_x=-x + [ir_distances_wf(1,:)] ;
            u_i_y = -y + [ir_distances_wf(2,:)] ; 
            u_i = [sensor_gains .* u_i_x ; sensor_gains .* u_i_y];
            u_ao = sum(u_i,2);  %% Returns a coloumn vector containing the sum of each row
...
%             fprintf('(v,w) = (%0.4g,%0.4g)\n', v,w);
        ...
        function ir_distances_wf = apply_sensor_geometry(obj, ir_distances, state_estimate)
            ...
            ir_distances_rf = zeros(3,n_sensors); %% Creating a blanck matrix
            for i=1:n_sensors
